  ...
  const loadGameSessions = async (gameName: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `${baseUrl}/sessions/tournament/${tournament.tournamentId}/child/${childId}`
      );
  ...
  const getGameSpecificMetrics = (session: GameSession, gameName: string) => {
    switch (gameName) {
      case "Dance Doodle":
        ...
        const totalDanceTime = completedDancePoses.reduce(
          (sum, pose) => sum + (session[pose] || 0),
          0
        );
        return {
          ...
          averageTime:
            completedDancePoses.length > 0
              ? totalDanceTime / completedDancePoses.length
              : 0,
      ...
      case "Mirror Posture Game":
        ...
        const totalPostureTime = completedPosturePoses.reduce(
          (sum, pose) => sum + (session[pose] || 0),
          0
        );
        return {
          ...
          averageTime:
            completedPosturePoses.length > 0
              ? totalPostureTime / completedPosturePoses.length
              : 0,
      ...
      case "Gesture Game":
        ...
        const totalGestureTime = completedGesturePoses.reduce(
          (sum, pose) => sum + (session[pose] || 0),
          0
        );
        return {
          ...
          averageTime:
            completedGesturePoses.length > 0
              ? totalGestureTime / completedGesturePoses.length
              : 0,
      ...
      case "Gaze Game":
        ...
        return {
          ...
          details: [
            {
              name: "Round 1 🎈",
              time: session.round1Count,
              completed:
                session.round1Count !== null &&
                session.round1Count !== undefined,
            },
            {
              name: "Round 2 🎈",
              time: session.round2Count,
              completed:
                session.round2Count !== null &&
                session.round2Count !== undefined,
            },
            {
              name: "Round 3 🎈",
              time: session.round3Count,
              completed:
                session.round3Count !== null &&
                session.round3Count !== undefined,
            },
          ],
        };
      ...
      case "Repeat With Me Game":
        ...
        const totalRepeatScore = completedRepeatRounds.reduce(
        ...
        return {
          ...
          averageTime:
            completedRepeatRounds.length > 0
              ? totalRepeatScore / completedRepeatRounds.length
              : 0,
              ...
              <CardContent>
                <div className="space-y-4">
                  {sessions.map((session, index) => {
                    const metrics = getGameSpecificMetrics(
                      session,
                      selectedGame
                    );
                          ...
                          <div className="text-right">
                            <div className="text-lg font-bold text-blue-600">
                              {selectedGame === "Gaze Game"
                                ...
                                ? `${Math.round(metrics.averageTime)}%`
                          ...
                          <div className="text-center p-2 bg-purple-50 rounded">
                            <div className="text-sm font-bold text-purple-600">
                              {selectedGame === "Gaze Game"
                                ...
                                : selectedGame === "Repeat With Me Game"
                                ? `${Math.round(
                                    metrics.totalTime / metrics.completed
                                  )}%`
