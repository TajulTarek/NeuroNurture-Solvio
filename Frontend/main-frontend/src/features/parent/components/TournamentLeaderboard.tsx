...
const TournamentLeaderboard: React.FC<TournamentLeaderboardProps> = ({
  ...
  const loadGameLeaderboard = async (gameName: string) => {
    ...
    try {
      const baseUrl =
        GAME_SERVICE_URLS[gameName as keyof typeof GAME_SERVICE_URLS];
      if (!baseUrl) {
      ...
      const response = await fetch(
        `${baseUrl}/sessions/tournament/${tournament.tournamentId}`
      );
      ...
      if (response.ok) {
        ...
        console.log(
          `🔍 [LEADERBOARD DEBUG] Number of sessions:`,
          sessions.length
        );

        const leaderboard = await calculateLeaderboard(sessions, gameName);
        console.log(
          `🔍 [LEADERBOARD DEBUG] Calculated leaderboard:`,
          leaderboard
        );
  ...
  const calculateLeaderboard = async (
    ...
    console.log(
      `🔍 [LEADERBOARD DEBUG] Calculating leaderboard for ${sessions.length} sessions`
    );
    ...
    console.log(
      `🔍 [LEADERBOARD DEBUG] Grouped sessions by child:`,
      Array.from(childSessions.keys())
    );
    ...
    childSessions.forEach((childSessionList, sessionChildId) => {
      ...
      const actualChildName =
        childNames[sessionChildId] ||
        bestSession.childName ||
        `Child ${sessionChildId}`;
      console.log(
        `🔍 [LEADERBOARD DEBUG] Child ${sessionChildId} name:`,
        actualChildName
      );
      console.log(
        `🔍 [LEADERBOARD DEBUG] Best session for ${actualChildName}:`,
        {
    ...
    leaderboard.sort((a, b) => {
      ...
      if (a.bestScore !== b.bestScore) {
        ...
        if (
          selectedGame === "Dance Doodle" ||
          selectedGame === "Mirror Posture Game" ||
          selectedGame === "Gesture Game"
        ) {
  ...
  const fetchChildNames = async (
    ...
    try {
      ...
      const promises = childIds.map(async (childId) => {
        try {
          ...
          if (response.ok) {
            ...
            console.log(
              `🔍 [LEADERBOARD DEBUG] Fetched name for child ${childId}:`,
              childData.name
            );
          } else {
            console.warn(
              `🔍 [LEADERBOARD DEBUG] Failed to fetch child ${childId}:`,
              response.status
            );
          ...
          console.error(
            `🔍 [LEADERBOARD DEBUG] Error fetching child ${childId}:`,
            error
          );
          childNames[childId] = `Child ${childId}`;
        }
      ...
      console.error(
        "🔍 [LEADERBOARD DEBUG] Error fetching child names:",
        error
      );
    }
  ...
  const calculateCompletionRate = (
    ...
    switch (gameName) {
      case "Dance Doodle":
        ...
        const completedDancePoses = dancePoses.filter(
          (pose) => session[pose] !== null && session[pose] !== undefined
        ).length;
      ...
      case "Mirror Posture Game":
        ...
        const completedPosturePoses = posturePoses.filter(
          (pose) => session[pose] !== null && session[pose] !== undefined
        ).length;
      ...
      case "Gesture Game":
        ...
        const completedGesturePoses = gesturePoses.filter(
          (pose) => session[pose] !== null && session[pose] !== undefined
        ).length;
      ...
      case "Gaze Game":
        ...
        const completedRounds = rounds.filter(
          (round) => session[round] !== null && session[round] !== undefined
        ).length;
      ...
      case "Repeat With Me Game":
        ...
        const completedRepeatRounds = repeatRounds.filter(
          (round) => session[round] !== null && session[round] !== undefined
        ).length;
      ...
      case "Gaze Game":
        return {
          totalBalloons:
            ...
            (session.round2Count || 0) +
            (session.round3Count || 0),
          roundsPlayed: ["round1Count", "round2Count", "round3Count"].filter(
            (round) => session[round] !== null && session[round] !== undefined
          ).length,
        };
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-600">
                            {selectedGame === "Dance Doodle" &&
                              `${entry.gameSpecificData.completedPoses}/${entry.gameSpecificData.totalPoses} poses`}
                            {selectedGame === "Mirror Posture Game" &&
                              `${entry.gameSpecificData.completedPostures}/${entry.gameSpecificData.totalPostures} postures`}
                            {selectedGame === "Gesture Game" &&
                              `${entry.gameSpecificData.completedGestures}/${entry.gameSpecificData.totalGestures} gestures`}
                            {selectedGame === "Gaze Game" &&
                              `${entry.gameSpecificData.roundsPlayed}/3 rounds • ${entry.gameSpecificData.totalBalloons} balloons`}
                            {selectedGame === "Repeat With Me Game" &&
                              `${entry.gameSpecificData.completedRounds}/${
                                entry.gameSpecificData.totalRounds
                              } rounds • ${Math.round(
                                entry.bestScore /
                                  entry.gameSpecificData.completedRounds
                              )}% avg`}
