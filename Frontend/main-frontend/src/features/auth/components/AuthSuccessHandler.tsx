    ...
    const checkParentInfo = async () => {
      ...
      try {
        ...
        const validAuthenticatedPages = [
          ...
          "/view-parent-info",
          "/games/gesture",
          "/games/mirror-posture",
        ];
        if (validAuthenticatedPages.includes(location.pathname)) {
          console.log(
            "AuthSuccessHandler: Already on valid authenticated page, not redirecting"
          );
          setIsChecking(false);
          onComplete();
        ...
        if (parentResponse.ok) {
          ...
          if (childrenResponse.ok) {
            ...
            console.log(
              "AuthSuccessHandler: Error getting children, redirecting to /children"
            );
            navigate("/children");
          }
        } else {
          ...
          console.log(
            "AuthSuccessHandler: No parent found, redirecting to /parent-info"
          );
        ...
        console.log(
          "AuthSuccessHandler: Error occurred, redirecting to /dashboard"
        );
        navigate("/dashboard");
      } finally {
        clearTimeout(timeoutId);
