...
const GestureRecognizerComponent: React.FC<GestureRecognizerComponentProps> = ({
  taskId,
  tournamentId,
}) => {
  ...
  const [gameSession, setGameSession] = useState<GestureGameSession | null>(
    null
  );
  ...
  const saveGameDataToBackend = useCallback(
    async (gameSession: GestureGameSession) => {
      try {
        ...
        gameSession.rounds.forEach((round, index) => {
          ...
          const gestureName = round.gestureName
            .toLowerCase()
            .replace(/\s+/g, "");
          console.log(
            "Mapping gesture:",
    ...
    () => [
      {
        ...
        label: "closed_fist",
        emoji: "✊",
        image: "/gesture_images/closed_fist.jpg",
        description: "Make a strong fist like a superhero!",
      },
      {
        name: "Open Palm",
        label: "open_palm",
        emoji: "✋",
        image: "/gesture_images/open_palm.jpg",
        description: "Show your palm like saying hello!",
      },
      {
        name: "Pointing Up",
        label: "pointing_up",
        emoji: "☝️",
        image: "/gesture_images/pointing_up.jpg",
        description: "Point your finger up to the sky!",
      },
      {
        name: "Thumbs Down",
        label: "thumbs_down",
        emoji: "👎",
        image: "/gesture_images/thumbs_down.jpg",
        description: "Show thumbs down like a judge!",
      },
      {
        name: "Thumbs Up",
        label: "thumbs_up",
        emoji: "👍",
        image: "/gesture_images/thumbs_up.jpg",
        description: "Give a thumbs up for good job!",
      },
      {
        name: "Victory",
        label: "victory",
        emoji: "✌️",
        image: "/gesture_images/victory.jpg",
        description: "Make a peace sign with your fingers!",
      },
      {
        name: "I Love You",
        label: "iloveyou",
        emoji: "🤟",
        image: "/gesture_images/iloveyou.jpg",
        description: "Show the love sign with your hand!",
      },
      {
        name: "Butterfly",
        label: "butterfly",
        emoji: "🦋",
        image: "/gesture_images/butterfly.jpg",
        description: "Flap your hands like a beautiful butterfly!",
      },
      {
        name: "Dua",
        label: "dua",
        emoji: "🤲",
        image: "/gesture_images/dua.jpg",
        description: "Hold your hands together in prayer position!",
      },
      {
        name: "Heart",
        label: "heart",
        emoji: "❤️",
        image: "/gesture_images/heart.jpg",
        description: "Make a heart shape with your hands!",
      },
      {
        name: "Spectacle",
        label: "spectacle",
        emoji: "🕶️",
        image: "/gesture_images/spectacle.jpg",
        description: "Make circles with your fingers like glasses!",
      },
    ],
    []
  );
  ...
  const handleRoundEndDirect = useCallback(() => {
    ...
    if (isCorrectRef.current === null) {
      ...
      if (!targetGestureRef.current) {
        console.error(
          "No target gesture set for failed round",
          currentRoundRef.current
        );
        return;
      }

      const currentGesture = gestures.find(
        (g) => g.label === targetGestureRef.current
      );
      if (currentGesture && gameSession) {
        ...
        setGameSession((prev) => {
          if (prev) {
            ...
            const roundExists = prev.rounds.some(
              (round) => round.roundNumber === currentRoundRef.current
            );
            if (roundExists) {
              console.log(
                `Round ${currentRoundRef.current} already exists, skipping duplicate entry`
              );
              return prev;
            }
        ...
        console.error(
          "Failed to record round: missing gesture or game session",
          {
            targetGesture: targetGestureRef.current,
            currentGesture,
  ...
  const startNextRound = useCallback(() => {
    ...
    if (currentRoundRef.current < 11) {
      ...
      const availableGestures = gestures.filter(
        (g) => !usedGesturesRef.current.includes(g.label)
      );
      console.log(
        "Available gestures for next round:",
        availableGestures.map((g) => g.label)
      );
      ...
      console.log(
        `🎯 Round ${nextRound}: Target gesture set to ${randomGesture.label}`
      );
  ...
  const handleGestureDetected = useCallback(
    (gesture: string, confidence: number) => {
      ...
      if (gesture === targetGestureRef.current && confidence >= 0.65) {
        // Changed threshold from 0.8 to 0.65 (65%)
        ...
        if (!targetGestureRef.current) {
          console.error(
            "No target gesture set for round",
            currentRoundRef.current
          );
        ...
        if (currentGesture && gameSession) {
          ...
          const roundAlreadyExists = gameSession?.rounds.some(
            (round) => round.roundNumber === currentRoundRef.current
          );
          if (roundAlreadyExists) {
            ...
            setGameSession((prev) => {
              if (prev) {
                console.log(
                  "Adding successful round to game session:",
                  roundStats
                );
                return {
                  ...prev,
      ...
      <div className="h-full flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-lg text-muted-foreground font-comic">
            Preparing gesture recognition system
          </p>
        </div>
      </div>
    );
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="card-playful border-4 border-primary bg-gradient-to-r from-primary/10 to-secondary/10 p-8 mb-8">
              ...
              <p className="text-xl text-muted-foreground leading-relaxed font-comic text-center">
                Hand Gesture Adventure helps you practice making different hand
                gestures! You'll see a big picture showing how to make a
                gesture, and then you copy it with your hand. It's like playing
                copycat with your hands! ✨
              </p>
            </div>
            <div className="grid md:grid-cols-2 gap-6 mb-8">
              <div className="card-playful border-2 border-fun-purple/20 p-6 text-center hover:scale-105 transition-all duration-300">
                ...
                <p className="text-lg text-muted-foreground font-comic">
                  We'll show you a big, colorful picture of how to make a hand
                  gesture
                </p>
              </div>
              ...
              <div className="card-playful border-2 border-fun-green/20 p-6 text-center hover:scale-105 transition-all duration-300">
                ...
                <p className="text-lg text-muted-foreground font-comic">
                  When you make the right gesture, you get a point and hear a
                  happy sound!
                </p>
              </div>
              <div className="card-playful border-2 border-fun-yellow/20 p-6 text-center hover:scale-105 transition-all duration-300">
                ...
                <p className="text-lg text-muted-foreground font-comic">
                  Try to copy 11 different gestures. You have 10 seconds for
                  each one!
                </p>
              </div>
            </div>
                ...
                <div className="w-full h-full bg-gradient-to-br from-purple-600 via-pink-500 to-blue-600 rounded-2xl shadow-2xl border-4 border-primary relative overflow-hidden">
                  ...
                  <div className="relative z-10 w-full h-full flex flex-col items-center justify-center text-white p-4">
                    ...
                    <div className="text-2xl font-playful mb-2 text-center drop-shadow-lg">
                      Final Score:{" "}
                      {gameSession
                        ? gameSession.rounds.filter((r) => r.completed).length
                        : score}
                      /11
                    </div>

                    {(() => {
                      const completedCount = gameSession
                        ? gameSession.rounds.filter((r) => r.completed).length
                        : score;
                      return (
                        <>
                          ...
                          <div className="text-xs font-comic text-center opacity-90 drop-shadow-sm mb-3">
                            {completedCount === 11
                              ? "Incredible performance! You've mastered all gestures!"
                              : completedCount >= 8
                              ? "Excellent work! You're on your way to becoming a gesture expert!"
              ...
              {!gameStarted && !gameEnded && (
                <div className="card-playful border-4 border-primary bg-gradient-to-r from-primary/10 to-secondary/10 p-6 text-center w-full h-full flex flex-col justify-center">
                  ...
                  <p className="text-lg text-muted-foreground mb-6 leading-relaxed font-comic">
                    Test your reflexes! You'll have 11 rounds to perform the
                    correct gesture within 10 seconds each.
                  </p>
                    ...
                    {isRoundCountdownActive ? (
                      ...
                      <>
                        <div className="flex justify-center items-center w-full h-full">
                          <img
                            src={
                              gestures.find((g) => g.label === targetGesture)
                                ?.image
                            }
                            alt={
                              gestures.find((g) => g.label === targetGesture)
