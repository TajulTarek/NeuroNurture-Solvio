...
export default function MirrorPostureGameInsights() {
      ...
      if (response.ok) {
        ...
        if (Array.isArray(sessions) && sessions.length > 0) {
          ...
          const processedSessions = sessions
            .map((session: any) => {
              const totalTime =
                (session.lookingLeft || 0) +
                (session.lookingRight || 0) +
                (session.mouthOpen || 0) +
                (session.showingTeeth || 0) +
                (session.kiss || 0);

              const completedPostures = [
                session.lookingLeft,
                session.lookingRight,
                session.mouthOpen,
                session.showingTeeth,
                session.kiss,
              ].filter((time) => time && time > 0).length;

              const accuracy =
                completedPostures > 0 ? (completedPostures / 5) * 100 : 0;
  ...
  const getPostureTime = (
    record: MirrorPostureGameRecord,
    postureName: string
  ): number | null => {
  ...
  const getPerformanceMetrics = () => {
    ...
    const averageSessionTime =
      sessionData.reduce((sum, session) => sum + session.totalTime, 0) /
      totalSessions;
    ...
    const averageAccuracy =
      sessionData.reduce((sum, session) => sum + session.accuracy, 0) /
      totalSessions;
    ...
    const improvementTrend =
      recentSessions.length > 1
        ? ((recentSessions[0].totalTime -
            recentSessions[recentSessions.length - 1].totalTime) /
            recentSessions[0].totalTime) *
          100
        : 0;
  ...
  const getConsistencyData = () => {
    return POSTURE_NAMES.map((name, index) => {
      ...
      const consistencyScore =
        completionRatio > 0
          ? Math.min(100, completionRatio + (averageTime > 0 ? 20 : 0))
          : 0;
  ...
  const smallestAvgTime =
    validAverageTimes.length > 0
      ? validAverageTimes.reduce((min, current) =>
          current.averageTime < min.averageTime ? current : min
        )
      : null;
  const highestAvgTime =
    validAverageTimes.length > 0
      ? validAverageTimes.reduce((max, current) =>
        ...
        <div className="text-center py-12">
          <div className="mb-8">
            ...
            <p className="text-xl font-comic text-gray-600 max-w-2xl mx-auto">
              Master facial expressions with fun challenges! Play new games and
              track your progress.
            </p>
          </div>
        </div>
          ...
          <Card className="card-playful backdrop-blur-sm bg-white/90 border-2 border-yellow-200 shadow-xl">
            <CardContent className="p-12 text-center">
              ...
              <p className="text-lg font-comic text-muted-foreground mb-8 max-w-md mx-auto">
                ...
                Start playing to unlock amazing insights and track your
                progress!
              </p>
              <Button
            ...
            <Tabs defaultValue="overview" className="space-y-8">
                  ...
                  <div className="relative bg-gradient-to-br from-white/95 to-white/85 backdrop-blur-sm rounded-3xl border-2 border-blue-200/50 shadow-2xl p-8">
                    <div className="text-center mb-8">
                      ...
                      <p className="text-lg font-comic text-gray-600">
                        Let's see how {selectedChild.name} is doing with facial
                        expressions!
                      </p>
                    </div>
                      ...
                      <div className="group relative">
                        ...
                        <div className="relative bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl border-2 border-blue-200 p-6 text-center hover:scale-105 transition-all duration-300 shadow-lg hover:shadow-2xl">
                          ...
                          <div className="text-4xl font-bold text-blue-600 mb-2">
                            {performanceMetrics?.averageSessionTime
                              ? performanceMetrics.averageSessionTime.toFixed(1)
                              : "0"}
                            s
                          </div>
                        ...
                        <div className="relative bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl border-2 border-purple-200 p-6 text-center hover:scale-105 transition-all duration-300 shadow-lg hover:shadow-2xl">
                          ...
                          <div className="text-4xl font-bold text-purple-600 mb-2">
                            {performanceMetrics?.averageAccuracy
                              ? performanceMetrics.averageAccuracy.toFixed(0)
                              : "0"}
                            %
                          </div>
                          ...
                          {performanceMetrics?.bestSession ? (
                            <>
                              <div className="text-3xl font-bold text-emerald-600 mb-2">
                                {performanceMetrics.bestSession.totalTime.toFixed(
                                  1
                                )}
                                s
                              </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="relative bg-gradient-to-br from-green-50/90 to-emerald-50/90 backdrop-blur-sm rounded-3xl border-2 border-green-200/70 shadow-xl hover:shadow-2xl transition-all duration-300 overflow-hidden">
                      ...
                      <div className="p-8 relative z-10">
                        <div className="text-center">
                          ...
                          <h3 className="text-2xl font-playful text-green-700 mb-6">
                            Fastest Posture
                          </h3>
                  ...
                  <div className="group relative">
                    ...
                    <div className="relative bg-gradient-to-br from-red-50/90 to-pink-50/90 backdrop-blur-sm rounded-3xl border-2 border-red-200/70 shadow-xl hover:shadow-2xl transition-all duration-300 overflow-hidden">
                      {/* Decorative elements */}
                      <div className="absolute top-4 right-4 text-6xl opacity-20 animate-wiggle">
                        📈
                      </div>
                      ...
                      <div className="p-8 relative z-10">
                        <div className="text-center">
                          ...
                          <h3 className="text-2xl font-playful text-red-700 mb-6">
                            Needs Practice
                          </h3>
                  ...
                  <div className="relative bg-gradient-to-br from-yellow-50/90 to-orange-50/90 backdrop-blur-sm rounded-3xl border-2 border-yellow-200/70 shadow-xl p-8">
                    <div className="text-center">
                      ...
                      <p className="text-lg font-comic text-orange-600 mb-6 max-w-2xl mx-auto">
                        Every practice session makes you better at facial
                        expressions. You're doing amazing, {selectedChild.name}!
                        🎉
                      </p>
              ...
              <TabsContent value="performance" className="space-y-8">
                ...
                <div className="relative">
                  ...
                  <div className="relative bg-gradient-to-br from-white/95 to-white/85 backdrop-blur-sm rounded-3xl border-2 border-blue-200/50 shadow-2xl p-8">
                    <div className="text-center mb-8">
                      ...
                      <p className="text-lg font-comic text-gray-600">
                        Deep dive into {selectedChild.name}'s facial expression
                        mastery!
                      </p>
                    </div>
                  </div>
                      ...
                      <div className="p-8 relative z-10">
                        <div className="text-center mb-6">
                          ...
                          <p className="text-sm font-comic text-blue-600">
                            How long it takes to complete each posture on
                            average
                          </p>
                        </div>
                    ...
                    <div className="relative bg-gradient-to-br from-purple-50/90 to-pink-50/90 backdrop-blur-sm rounded-3xl border-2 border-purple-200/70 shadow-xl hover:shadow-2xl transition-all duration-300 overflow-hidden">
                      ...
                      <div className="p-8 relative z-10">
                        <div className="text-center mb-6">
                          ...
                          <h3 className="text-2xl font-playful text-purple-700 mb-2">
                            Completion Ratios
                          </h3>
                        ...
                        <div className="space-y-4">
                          {averageTimesData
                            ...
                            .sort(
                              (a, b) => b.completionRatio - a.completionRatio
                            )
                            .map((item, index) => (
                              <div
                                key={item.posture}
                                className="group/item bg-white/70 rounded-xl border border-purple-200/50 p-4 hover:scale-105 transition-all duration-300 hover:shadow-lg"
                              >
                                <div className="flex items-center justify-between mb-3">
                                  <div className="flex items-center gap-2">
                                    <span className="text-2xl">
                                      {item.posture.split(" ").pop()}
                                    </span>
                ...
                <div className="group relative">
                  ...
                  <div className="relative bg-gradient-to-br from-green-50/90 to-emerald-50/90 backdrop-blur-sm rounded-3xl border-2 border-green-200/70 shadow-xl hover:shadow-2xl transition-all duration-300 overflow-hidden">
                    ...
                    <div className="absolute top-1/2 left-4 text-3xl opacity-20 animate-pulse">
                      💪
                    </div>
                    ...
                    <div className="p-8 relative z-10">
                      <div className="text-center mb-6">
                        ...
                        <p className="text-sm font-comic text-green-600">
                          Track your improvement over time - lower times mean
                          better performance!
                        </p>
                      </div>
                      ...
                      <div className="bg-white/70 rounded-2xl p-4 border border-green-200/50">
                        {sessionData.length > 0 ? (
                          ...
                          <div className="text-center text-muted-foreground p-8">
                            ...
                            <div className="text-xl font-comic text-green-600 mb-2">
                  ...
                  <div className="group relative">
                    ...
                    <div className="relative bg-gradient-to-br from-orange-50/90 to-yellow-50/90 backdrop-blur-sm rounded-3xl border-2 border-orange-200/70 shadow-xl hover:shadow-2xl transition-all duration-300 overflow-hidden">
                      ...
                      <div className="p-8 relative z-10">
                        <div className="text-center">
                          <div className="text-5xl mb-4 animate-bounce">📈</div>
                          <h3 className="text-2xl font-playful text-orange-700 mb-6">
                            Improvement Trend
                          </h3>
                  ...
                  <div className="group relative">
                    ...
                    <div className="relative bg-gradient-to-br from-pink-50/90 to-rose-50/90 backdrop-blur-sm rounded-3xl border-2 border-pink-200/70 shadow-xl hover:shadow-2xl transition-all duration-300 overflow-hidden">
                      ...
                      <div className="p-8 relative z-10">
                        <div className="text-center">
                          <div className="text-5xl mb-4 animate-wiggle">🥧</div>
                          <h3 className="text-2xl font-playful text-pink-700 mb-6">
                            Posture Distribution
                          </h3>
                          ...
                          <div className="bg-white/70 rounded-2xl p-4 border border-pink-200/50">
                            {postureDistributionData.length > 0 ? (
                              ...
                              <div className="text-center text-muted-foreground p-4">
                                ...
                                <div className="text-lg font-comic text-pink-600">
                                  No data available yet
                                </div>
                                <div className="text-sm text-pink-500">
                                  Start playing to see the distribution!
                                </div>
                              </div>
                            )}
                          </div>
                  ...
                  <div className="relative bg-gradient-to-br from-indigo-50/90 to-purple-50/90 backdrop-blur-sm rounded-3xl border-2 border-indigo-200/70 shadow-xl p-8">
                    <div className="text-center">
                      ...
                      <p className="text-lg font-comic text-indigo-600 mb-6 max-w-2xl mx-auto">
                        {selectedChild.name} is showing amazing progress in
                        facial expressions! Keep practicing to improve even
                        more! 🌟
                      </p>
                      <div className="flex justify-center space-x-4 text-2xl">
                        <span
                        ...
                        <div className="space-y-4">
                          {POSTURE_NAMES.map((postureName) => {
                            ...
                            if (lastSession && lastSession.postureTimes) {
                              lastSessionTime =
                                lastSession.postureTimes[postureName] || null;
                            }
                            ...
                            const isCompleted =
                              lastSessionTime && lastSessionTime > 0;
                            const difference = isCompleted
                              ? lastSessionTime - averageTime
                              : 0;
                            const differenceAbs = Math.abs(difference);
                            const isImproving = isCompleted && difference < 0;
                            const percentageChange = isCompleted
                              ? Math.abs((difference / averageTime) * 100)
                              : 0;

                            return (
                              <div
                                key={postureName}
                                className="p-4 bg-gradient-to-r from-purple-50 to-blue-50 rounded-xl border border-purple-200 hover:scale-105 transition-all"
                              >
                                <div className="flex items-center justify-between mb-3">
                                  <div className="flex items-center gap-2">
                                    <span className="text-2xl">
                                      {postureName.split(" ").pop()}
                                    </span>
                                    <span className="font-comic text-base font-medium">
                                      {postureName
                                        .split(" ")
                                        .slice(0, -1)
                                        .join(" ")}
              ...
              <TabsContent value="consistency" className="space-y-6">
                <Card className="card-playful backdrop-blur-sm bg-white/90 border-2 border-blue-200 shadow-xl">
                  <CardHeader className="pb-3">
                    <CardTitle className="font-playful text-xl text-blue-600">
              ...
              <TabsContent value="history" className="space-y-6">
                <Card className="card-playful backdrop-blur-sm bg-white/90 border-2 border-purple-200 shadow-xl">
                  <CardHeader className="pb-3">
                    ...
                    <CardDescription className="font-comic text-base">
                      Your recent performance history with detailed posture
                      breakdowns
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      {gameHistory.map((record, index) => {
                        const totalTime = POSTURE_NAMES.reduce(
                          (sum, posture) => {
                              ...
                              <div className="grid grid-cols-2 gap-2">
                                {POSTURE_NAMES.map((posture) => {
                                  ...
                                  const name = posture
                                    .split(" ")
                                    .slice(0, -1)
                                    .join(" "); // Get the name without emoji
                    ...
                    {totalPages > 1 && (
                      <div className="text-center mt-4 text-sm text-gray-600">
                        ...
                        {Math.min(5, gameHistory.length)} of{" "}
                        {gameHistory.length} sessions
                      </div>
                    )}
                  </CardContent>
                </Card>
