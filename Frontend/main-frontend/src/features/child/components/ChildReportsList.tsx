...
const getVerdictDisplay = (verdict: string | null) => {
  switch (verdict) {
    case 'SCREENING_NEEDED':
      return {
        label: 'Offline Screening Recommended',
        description: 'The doctor recommends an in-person evaluation.',
    ...
    case 'INCONCLUSIVE':
      return {
        ...
        description: 'The doctor needs more game sessions for assessment.',
...
export default function ChildReportsList({ childId }: ChildReportsListProps) {
  ...
  const loadReports = async () => {
    try {
      setIsLoading(true);
      setError(null);
      const data = await reportService.getChildReports(childId);
      setReports(data);
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Reports sent to doctors for analysis</p>
        </div>
        <Button onClick={() => navigate('/child/send-report')} className="bg-blue-600 hover:bg-blue-700">
        ...
        <Card className="border-dashed">
          <CardContent className="py-12 text-center">
            ...
            <p className="text-gray-500 mb-4">
              Send your child's game performance data to a doctor for professional analysis.
            </p>
            <Button onClick={() => navigate('/child/send-report')} className="bg-blue-600 hover:bg-blue-700">
              <Send className="w-4 h-4 mr-2" />
              Send First Report
                      ...
                      <div className="p-4 bg-gray-50">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {selectedGames.map((gameId) => (
                            <span
                              key={gameId}
                              className="px-3 py-1.5 bg-gradient-to-r from-blue-100 to-purple-100 text-blue-700 rounded-full text-sm font-medium flex items-center gap-1.5"
                            >
                          ...
                          <div className="flex items-start space-x-3">
                            ...
                            <div className="flex-1">
                              ...
                              {report.verdict && (
                                <div className="mb-4">
                                  {(() => {
                                    ...
                                    return (
                                      <div className={`p-4 rounded-lg border-2 ${verdictInfo.color} ${verdictInfo.bgColor}`}>
                                        <div className="flex items-center space-x-3">
                                          <VerdictIcon className="w-6 h-6" />
                                          <div>
                                            <h6 className="font-bold text-lg">{verdictInfo.label}</h6>
                                            <p className="text-sm opacity-80">{verdictInfo.description}</p>
                                          </div>
                                        </div>
                                      </div>
                                    );
                              ...
                              <div className="bg-white p-4 rounded-lg border border-green-200">
                                <h6 className="text-sm font-medium text-gray-600 mb-2">Analysis & Comments</h6>
                        ...
                        <div className="p-4 bg-yellow-50 border-t border-yellow-200">
                          <div className="flex items-center space-x-3">
                            ...
                            <p className="text-yellow-700">
                              Awaiting doctor's response. You'll be notified when the doctor reviews
                              this report.
                            </p>
                          </div>
                        </div>
