...
export class DoctorTaskService {
  ...
  static async createTasks(request: DoctorTaskCreateRequest, doctorId: number): Promise<DoctorTaskResponse[]> {
    ...
    console.log('JWT Token:', localStorage.getItem('doctorToken') ? 'Present' : 'Missing');
  ...
  static async getTasksByDoctor(doctorId: string): Promise<DoctorTaskResponse[]> {
  ...
  static async updateTaskStatus(taskId: string, status: string): Promise<DoctorTaskResponse> {
